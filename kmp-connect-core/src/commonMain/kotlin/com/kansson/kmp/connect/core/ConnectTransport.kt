        ...
        when (response.status) {
            HttpStatusCode.OK -> {
                ...
                ResponseMessage.Failure(
                    ...
                    details = error?.details.orEmpty().map {
                        com.kansson.kmp.connect.core.wkt.Any(
                            typeUrl = "type.googleapis.com/${it.type}",
                            value = Error.Detail.Base64.decode(it.value),
                        )
                    },
                    headers = responseHeaders,
                    trailers = trailers,
